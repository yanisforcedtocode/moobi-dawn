...
type CheckFunction = (dateRange: DateRange, exclude: number[] | undefined) => {
    checkPositive: boolean,
    numberOfPosDays: number, posDays: number[],
    numberOfDays: number, newDateRange: DateRange, checkType: string
}
type CheckResult = {
    checkPositive: boolean,
    numberOfPosDays: number, posDays: number[],
...
// main class
class CalETA{
    vcalendar: VCalendarItem[];
    holidayArr: number[];
    holidayArrWSat: number[];
    today: number;
    eta: number;
    etaThresholdDays: number;
    msPerDay: number;
    renderSectionId: string;
    countDownInstance: TimeCountDwn;
    bulk: boolean;
    refresh: boolean
    constructor(eta: number, renderSectionId:string, countDownInstance: TimeCountDwn, etaThresholdDays: number, refresh: boolean){
        // assign variables
        this.msPerDay = 86400000
        this.renderSectionId = renderSectionId
        this.etaThresholdDays = etaThresholdDays
        const bulk = (document.getElementById('product-ETA--raw')! as HTMLElement).dataset.bulk
        this.bulk = ((bulk)=>{
            if(bulk !== undefined){
                if(bulk === "true"){
                    return true
    ...
    private getholidayArray(holidays: VCalendarItem[], noSatDays: boolean, noSunDays: boolean){
    ...
    // format
    private newVCalDate(vCalDate: string) {
        if(!/^(\d){8}$/.test(vCalDate)) return "invalid date";
    ...
    private parseDateMS(date: Date){
        const ms = date.valueOf()
        return ms
    }

    private getETARange(eta: number, days: number){
    ...
    private getRange(beginDate:Date, days: number){
    ...
    private getNewDateRange(startDate: number, numberOfDays: number){
    ...
    // checks
    private checkSatStart: CheckFunction = (dateRange: DateRange, exclude: number[] = [])=>{
    ...
    private checkSunStart: CheckFunction =(dateRange: DateRange, exclude: number[] = [])=>{
    ...
    private checkHoliaysStart: CheckFunction =(dateRange: DateRange, exclude: number[] = [])=>{
    ...
    private checkSatEnd: CheckFunction = (dateRange: DateRange, exclude: number[] = [])=>{
        const checkType = "saturdays"
        let dateCount = dateRange.startDate
        let checkPositive = false
        let numberOfPosDays = 0
    ...
    private checkSunEnd: CheckFunction =(dateRange: DateRange, exclude: number[] = [])=>{
        const checkType = "sundays"
        let dateCount = dateRange.startDate
        let checkPositive = false
        let numberOfPosDays = 0
    ...
    private checkHoliaysEnd: CheckFunction =(dateRange: DateRange, exclude: number[] = [])=>{
        const checkType = "holidays"
        let dateCount = dateRange.startDate
        let checkPositive = false
        let numberOfPosDays = 0
    ...
    // check loop
    private loopChecks(checks: CheckFunction[], eta: number, days: number){
        let dateRange = this.getETARange(eta, days)
        let returnObj = <CheckResult>{}
        ...
        for(let i = 0 ; i<loopCount; i++){
            ...
            checks.forEach((el, ind)=>{
                const checkresult: CheckResult = el(dateRange, excludeArr)
    ...
    // render result
    private renderResult(today: number, extendedRange: {start: Date; end: Date}, threshold: number, badge:boolean){
    ...
    private refreshETA(delay: number){
    ...
    // composition
    private getExtendedRange(eta: number, days: number){
        ...
        const checkResult_start: CheckResult = this.loopChecks(testsArrStart, eta, 2)
        const checkResult_end: CheckResult = this.loopChecks(testsArrEnd, checkResult_start.newDateRange.endDate, days)
...
class TimeCountDwn{
    countDownToHour: number;
    countDownTomin: number;
    msPerDay: number;
    msPerSec: number;
    countDownId: string;
    enableCountDown: boolean;
    constructor(countDownToHour: number, countDownToMin:number, countDownId: string, enableCountDown: boolean){
        // assign variables
        this.msPerDay = 86400000
        this.msPerSec = 1000
    ...
    private getCountDownTime(targetTime: number = this.getTargetTime()){
        ...
        const convert2Digits = (value: number)=>{
            let string2Digits = ''
        ...
        const countDwnStringObj: CountDwnStringObj = {
    ...
    private setCountDownIntv(interval: number, targetTimeFn: Function, refreshFn: Function){
    ...
    // render
    private renderCountDwnSection(targetSectionId: string){
    ...
    private refreshCountDwn(countDownStringObj: CountDwnStringObj){
